...
import { SYSTEM_INSTRUCTION } from './dataBot';
import { SiGooglegemini } from 'react-icons/si';
import { GoogleGenAI } from '@google/genai';

const KEY_1 = process.env.NEXT_PUBLIC_GEMINI_API_KEY_1;
const KEY_2 = process.env.NEXT_PUBLIC_GEMINI_API_KEY_2;
const KEY_3 = process.env.NEXT_PUBLIC_GEMINI_API_KEY_3;

const API_KEYS: string[] = [KEY_1, KEY_2, KEY_3].filter((key): key is string => !!key);
...
const ChatbotMini: React.FC = () => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    setMessages((prev) => {
      const updated = [...prev, { text: userMessage, sender: 'user' as const }];
    ...
    if (botResponseText) {
      setMessages((prev) => {
        const updated = [...prev, { text: botResponseText, sender: 'bot' as const }];
        if (updated.length > MAX_HISTORY) {
          updated.shift();
        }
        return updated;
          ...
          <div className="flex-grow p-3 overflow-y-auto space-y-3 text-sm">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-gray-200 text-gray-600 p-2 rounded-lg max-w-[85%]">AI lagi ngetik bro...</div>
              </div>
            )}
          </div>
